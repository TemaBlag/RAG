...
class AgenticChunker:
    ...
    def _find_relevant_chunk(self, proposition):
        ...
        responce = self.llm.invoke(chunk_found).content
        if not responce.find('No chunks'):
            chunk_found = responce
    ...
    def get_chunks(self, get_type='dict'):
        ...
        if get_type == 'list_of_strings':
            chunks = []
            for chunk_id, chunk in self.chunks.items():
                chunks.append(" ".join([str(x) for x in chunk['propositions']]))
            return chunks
